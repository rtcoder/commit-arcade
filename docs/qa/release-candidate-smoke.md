...
The AMO source ZIP is intentionally omitted from this table because it contains this QA document. Record its checksum
from `shasum -a 256 dist/commit-arcade-firefox-source-v1.0.0.zip` after the final source package is generated.
...
- Safari store packaging is outside the automated WebExtension workflow until an Xcode/App Store packaging path is
  implemented.
