...
**Architecture:** Keep each game as an isolated `CommitArcadeGame` module with deterministic options for tests. Wire the new modules through the existing registry, picker, factory, input filtering, HUD help and static games documentation.
...
- [ ] Write failing tests for start rendering, player movement, projectile collision scoring and invader descent game over.
